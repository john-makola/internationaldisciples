...
const PrayerRequest = () => {
  ...
  const FORMSPARK_ACTION_URL = "https://submit-form.com/KdOjRHB3Y";

  const validate = () => {
    const newErrors: Record<string, string> = {};
    if (!formData.name) newErrors.name = "Name is required";
    if (!formData.email) newErrors.email = "Email is required";
    if (!formData.title) newErrors.title = "Prayer Title is required";
    if (!formData.message) newErrors.message = "Prayer Request is required";
    return newErrors;
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {submitted && (
              <p className="text-green-600 font-semibold mb-4">
                Thank you! We have received your prayer request.
              </p>
            )}
            ...
            <input
              type="text"
              placeholder="Organization or Group (Optional)"
              className="w-full border p-3 rounded"
